...
- Location: tests\API\transactionValidation.spec.ts:5:5
...
```ts
  ...
  4  | 
  5  | test('Validate Transactions', async ({ request }) => {
  ...
  11 |     const response = await request.get(`https://parabank.parasoft.com/parabank/services/bank/accounts/${savingAcc.accountId}/transactions`
  12 |     )
